...
class WeightedRandom:
	def __init__(self):
		random.seed()
	def GetWeightedChoice(self, choiceDict):
...
class Hand:
	...
	def Throw(self):
		...
		playerChoice = input("Choose Rock, Paper, or Scissors (or Exit to quit): ").upper()
		if playerChoice != "ROCK" and playerChoice != "PAPER" and playerChoice != "SCISSORS" and playerChoice != "EXIT" and playerChoice != "QUIT" and playerChoice != "PRINTDICT":
			print("{0} is not a valid choice.".format(playerChoice))
			return True
		if playerChoice == "PRINTDICT":
			self.choiceMaker.PrintTrackerDict()
			return True
		if playerChoice == "EXIT" or playerChoice == "QUIT":
			return False
		print()
		print("  YOU        COM")
